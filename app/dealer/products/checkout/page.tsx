...
export default function DealerCheckoutPage() {
  ...
  const createOrder = async () => {
    ...
    try {
      setIsLoading(true);
      const res = await processCheckout({ shippingAddress: currentFormData, paymentMethod: 'paypal' });
      
      if (!res.data.paypalOrderId) throw new Error('Invalid response from server');
    ...
    <PayPalScriptProvider options={{ clientId: process.env.NEXT_PUBLIC_PAYPAL_CLIENT_ID || '', currency: "USD", intent: "capture" }}>
      <div className="min-h-screen bg-transparent pb-20">
        ...
        <h1 className="text-3xl font-black text-slate-900 tracking-tight mb-8 flex items-center gap-3">
          <ShieldCheck className="w-8 h-8 text-blue-500" /> Secure Dealer Checkout
        </h1>
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
              <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2">
                <Truck className="w-5 h-5 text-blue-500" /> Shipping Destination
              </h2>
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <input type="text" name="street" value={formData.street} onChange={handleInputChange} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none" required />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1.5">City</label>
                    <input type="text" name="city" value={formData.city} onChange={handleInputChange} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none" required />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1.5">State</label>
                    <input type="text" name="state" value={formData.state} onChange={handleInputChange} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none" required />
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1.5">Pincode</label>
                    <input type="text" name="pincode" value={formData.pincode} onChange={handleInputChange} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none" required />
                  </div>
                  <div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
              ...
              <div className="p-4 rounded-xl border-2 border-blue-500 bg-blue-50 flex items-center gap-4 cursor-pointer">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-slate-500 font-medium">Credit Card, Debit Card, PayPal Wallet</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5">
            <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm sticky top-8">
              ...
              <div className="space-y-4 mb-6 border-t border-slate-100 pt-6">
                ...
                <div className="flex items-center justify-between text-sm font-medium text-slate-600">
                  <span>Tax (18%)</span><span className="font-bold text-slate-900">${tax.toLocaleString()}</span>
                </div>
              </div>
