...
import { MessageSquare, Send, User, Building2, Search } from 'lucide-react';
      ...
      <div className="flex-shrink-0 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">Communicate with dealers regarding your orders or inquiries</p>
          </div>
        </div>
      </div>
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${contact.role === 'DEALER' ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 text-slate-600'}`}>
                        {contact.role === 'DEALER' ? <Building2 className="w-5 h-5" /> : <User className="w-5 h-5" />}
                      </div>
              ...
              <div className="flex-shrink-0 p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/50">
                <div className="flex items-center gap-4 min-w-0">
                  <div className="w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 border border-slate-200 shadow-sm bg-white">
                    {activeContact.role === 'DEALER' ? <Building2 className="w-6 h-6 text-indigo-500" /> : <User className="w-6 h-6 text-slate-500" />}
                  </div>
                  <div>
                        ...
                        <div key={idx} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                          <div className={`flex items-end gap-2 max-w-[85%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm ${isMe ? 'bg-green-500 text-white' : 'bg-white border border-slate-200 text-slate-600'}`}>
                              {isMe ? <User className="w-4 h-4" /> : activeContact.role === 'DEALER' ? <Building2 className="w-4 h-4" /> : <User className="w-4 h-4" />}
                            </div>
